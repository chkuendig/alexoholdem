...
//	 * @see #getInstance(com.stevebrecher.poker.Card.Rank, com.stevebrecher.poker.Card.Suit)
	...
	/**
	 * Constructs a card of the specified rank and suit.
	 * @param rs a {@link String} of length 2, where the first character is in {@link Card2.Rank#RANK_CHARS} and
	 * 			 the second is in {@link Card2.Suit#SUIT_CHARS} (case insensitive).
	 * @throws IllegalArgumentException on the first character in rs which is not found in the respective string.
	 * @see #getInstance(String)
	 */
	public Card2(String rs) {
	...
	private final static Card2[]	theCards = new Card2[52];
	static {
		int i = 0;
		for (Suit s : Suit.values())
			for (Rank r : Rank.values())
				theCards[i++] = new Card2(r, s);
	}

	/**
	 * Returns a pre-existing instance of {@link Card2} of the specified rank and suit.
	 * @param rank a {@link Rank}
	 * @param suit a {@link Suit}
	 * @return an instance of {@link Card2} of the specified rank and suit.
	 */
	public static Card2 getInstance(Rank rank, Suit suit) {
	 ...
	 * @throws IllegalArgumentException on the first character in rs which is not found in the respective string.
	 */
	public static Card2 getInstance(String rs) {
		if (rs.length() != 2)
			throw new IllegalArgumentException('"' + rs + "\".length != 2");
		try {
	 ...
	 * @return a {@link String} of length 2 containing a character in {@link Card2.Rank#RANK_CHARS} denoting this
	 * 			card&#39;s rank followed by a character in {@link Card2.Suit#SUIT_CHARS} denoting this
	 * 			card&#39;s suit.
	 */
	@Override
	 ...
	 * @return <code>true</code> if the parameter is a {@link Card2} of the same rank and suit
	 * 			as this card; <code>false</code> otherwise.
	 */
	@Override
	public boolean equals (Object that) {
		if (!(that instanceof Card2)) return false;
		Card2 c = (Card2)that;
