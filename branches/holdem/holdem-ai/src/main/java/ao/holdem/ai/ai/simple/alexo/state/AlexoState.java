package ao.holdem.ai.ai.simple.alexo.state;
...
public class AlexoState
{
    ...
    private AlexoState(
            AlexoRound  nextRound,
            int         nextFirstCommit,
            int         nextLastCommit,
            int         nextRemainingRoundBets,
            boolean     nextFirstNextToAct,
            AlexoAction nextPrevActionInRound)
    {
        round              = nextRound;
    ...
    public AlexoState advance(AlexoAction act)
    {
        ...
        {
            return new AlexoState(
                    null, firstCommit, lastCommit, -1,
                    !firstToActNext, act);
        }
        ...
        else //if (act == AlexoAction.BET_RAISE)
        {
            return new AlexoState(
                    round,
                    firstCommit + ( firstToActNext ?  betSize() : 0),
                    lastCommit  + (!firstToActNext ?  betSize() : 0),
    ...
    public int deltas(AlexoCardSequence cards)
    {
        ...
        {
            int firstVal = AlexoHand.valueOf(
                    cards.firstHole(), cards.community());
